...
export function PatientsClient({ data }: { data: PaginatedPatients }) {
  ...
  const visiblePatients =
    activeTab === "all"
      ? data.patients
      : data.patients.filter((p) => p.status === activeTab);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        {data.canManage && (
          <Button
            ...
            className="gap-2 flex-shrink-0 self-start sm:self-auto"
          >
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Card className="px-4 py-3.5">
          <p className="text-2xl font-semibold tabular-nums text-slate-400">
            {statusCounts.inactive.toLocaleString()}
          </p>
            ...
            <table className="w-full min-w-[640px] text-sm">
              <thead>
                <tr className="border-b border-border bg-slate-50/60">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 hidden lg:table-cell">
                    Birth Date
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">
                    Status
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400 hidden xl:table-cell">
                    Created
                  </th>
                    ...
                    <td className="px-4 py-3.5">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${
                          STATUS_BADGE[patient.status] ?? STATUS_BADGE.active
                        }`}
                      >
