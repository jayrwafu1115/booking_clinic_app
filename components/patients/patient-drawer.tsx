      ...
      <SheetContent>
        ...
        <SheetHeader>
          <div className="flex items-center gap-3">
            ...
            >
              {isNew ? "New" : titleize(currentStatus)}
            </span>
          </div>
        </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <section className="space-y-3">
              ...
              <FormField label="Full name">
                <Input name="fullName" defaultValue={patient?.full_name ?? ""} required autoComplete="name" />
              </FormField>
              <div className="grid grid-cols-2 gap-3">
                <FormField label="Phone">
                  <Input name="phone" type="tel" defaultValue={patient?.phone ?? ""} required autoComplete="tel" />
                </FormField>
                <FormField label="Birth date">
                  <Input name="birthDate" type="date" defaultValue={patient?.birth_date ?? ""} autoComplete="bday" />
                </FormField>
                <FormField label="Email">
                  <Input name="email" type="email" defaultValue={patient?.email ?? ""} autoComplete="email" />
                </FormField>
                <FormField label="Gender">
                  ...
                  >
                    ...
                    {GENDERS.map((g) => (
                      <option key={g} value={g}>{titleize(g)}</option>
                    ))}
                  </select>
                </FormField>
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label="Address line 1">
                  <Input name="addressLine1" defaultValue={patient?.address_line_1 ?? ""} autoComplete="address-line1" />
                </FormField>
                <FormField label="Address line 2">
                  <Input name="addressLine2" defaultValue={patient?.address_line_2 ?? ""} autoComplete="address-line2" />
                </FormField>
                <FormField label="Barangay">
                  <Input name="barangay" defaultValue={patient?.barangay ?? ""} autoComplete="address-level3" />
                </FormField>
                <FormField label="City / Municipality">
                  <Input name="city" defaultValue={patient?.city ?? ""} autoComplete="address-level2" />
                </FormField>
                <FormField label="Province">
                  <Input name="province" defaultValue={patient?.province ?? ""} autoComplete="address-level1" />
                </FormField>
                <FormField label="Region">
                  <Input name="region" defaultValue={patient?.region ?? ""} autoComplete="off" />
                </FormField>
                <FormField label="Postal code">
                  <Input name="postalCode" defaultValue={patient?.postal_code ?? ""} autoComplete="postal-code" />
                </FormField>
              </div>
            </section>
            ...
            {/* Emergency & Notes */}
            <section className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Emergency & Notes</p>
              <div className="grid grid-cols-2 gap-3">
