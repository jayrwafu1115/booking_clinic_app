...
export async function createPatientDrawerAction(
  ...
  try {
    ...
    if (patientCount !== null && patientCount >= planFeatures.maxPatients) {
      return { message: `Your plan allows a maximum of ${planFeatures.maxPatients} patients. Upgrade to Pro to add more.` };
    }
    ...
    if (error || !data) return { message: error?.message ?? "Could not create patient." };
